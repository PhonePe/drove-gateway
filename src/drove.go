...
func eventStream() {
    go func() {
        ...
        for range ticker.C {
            ...
            req, err := http.NewRequest("GET", endpoint+"/apis/v1/events", nil)
            if err != nil {
                logger.WithFields(logrus.Fields{
                    "error":    err.Error(),
                    "endpoint": endpoint,
                }).Error("unable to create event stream request")
                go countDroveStreamErrors.Inc()
                continue
            }
            ...
            timer := time.AfterFunc(15*time.Second, func() {
                ...
                go countDroveStreamNoDataWarnings.Inc()
            })
            ...
            if err != nil {
                logger.WithFields(logrus.Fields{
                    "error":    err.Error(),
                    "endpoint": endpoint,
                }).Error("unable to access Drove event stream")
                go countDroveStreamErrors.Inc()
            ...
            for {
                ...
                // since ~10s seems to be the rate for dummy/keepalive events on the drove event stream
                ...
                if err != nil {
                    if err.Error() != "context canceled" {
                        logger.WithFields(logrus.Fields{
                            "error":    err.Error(),
                            "endpoint": endpoint,
                        }).Error("error reading Drove event stream")
                    }
                    go countDroveStreamErrors.Inc()
                    resp.Body.Close()
                    break
                ...
                logger.WithFields(logrus.Fields{
                    ...
                    "endpoint": endpoint,
                }).Info("drove event received")
                go countDroveEventsReceived.Inc()
                select {
                case eventqueue <- true: // add reload to our queue channel, unless it is full of course.
                default:
                    logger.Warn("queue is full")
